...
STATE_DICTS_TO_SERIALIZE = [
    "tests/fmlaas/data/mnist_cnn_2.pt",
    "tests/fmlaas/data/mnist_cnn.pt"]
OUTPUT_JSON = [
...
def serialize_state_dict(path):
    return {key: item.numpy().tolist()
            for key, item in torch.load(path).items()}
